...
export function EventsCalendar({ className = "" }: { className?: string }) {
  ...
  return (
    <div
      className={`flex h-full flex-col overflow-hidden bg-white shadow-[0_1px_0_rgba(11,31,58,0.06)] ring-1 ring-line ${className}`}
    >
      <div className="flex items-center justify-between gap-3 border-b border-line bg-soft px-4 py-4 md:px-6">
        <div className="flex min-w-0 flex-1 items-center justify-between gap-2 sm:justify-start sm:gap-3">
          <button
            ...
            className="display shrink-0 px-3 py-2 text-xs tracking-[0.14em] text-navy transition hover:bg-white"
            aria-label="Previous month"
          >
            Prev
          </button>
          <p className="display min-w-0 truncate text-center text-base tracking-[0.06em] text-navy sm:text-left sm:text-lg md:text-xl">
        ...
        <button
          ...
          className="display shrink-0 text-xs tracking-[0.14em] text-accent underline decoration-accent underline-offset-4"
        >
          Today
        </button>
      </div>

      <div className="flex gap-1 overflow-x-auto border-b border-line bg-soft/60 px-3 py-2 [-ms-overflow-style:none] [scrollbar-width:none] md:px-4 [&::-webkit-scrollbar]:hidden">
      ...
      <div className="grid min-h-0 flex-1 gap-0 md:grid-cols-[1.4fr_1fr]">
        <div className="order-2 border-t border-line bg-white p-6 md:border-l md:border-t-0 md:p-6">
          ...
          {selectedIsMeeting ? (
            <div className="mt-4">
              <Image
                ...
                width={1024}
                height={537}
                className="mb-4 h-6 w-auto rounded-[3px] object-contain"
              />
        ...
        <div className="order-1 bg-soft/40 p-4 md:p-5">
            ...
            {cells.map((date, index) => {
              if (!date) {
                return (
                  <div
                    key={`empty-${index}`}
                    className="aspect-square min-h-0 md:min-h-16 md:aspect-auto"
                  />
                );
              }
              ...
              return (
                <button
                  ...
                  className={`flex aspect-square min-h-0 flex-col items-center justify-center border p-1 transition md:aspect-auto md:min-h-16 md:items-start md:justify-start md:p-1.5 ${
                    isSelected
                ...
                >
                  <span
                    className={`display text-xs leading-none md:text-xs ${
                  ...
                  {isMeeting ? (
                    <>
                      <span
                        className={`mt-1 h-1.5 w-1.5 rounded-full md:hidden ${
                      ...
                      <Image
                        ...
                        width={1024}
                        height={537}
                        className="mt-1 hidden h-3.5 w-auto max-w-full rounded-[2px] object-contain md:block md:h-4"
                      />
                    </>
                  ) : null}
                  {isJulyMarker ? (
                    <span
                      className={`mt-1 block text-center text-[8px] font-semibold leading-none tracking-wide md:text-left md:text-[9px] ${
                        isSelected ? "text-white/80" : "text-accent"
                      }`}
                    >
                      <span className="md:hidden">—</span>
                      <span className="hidden md:inline">No mtg</span>
                    </span>
                  ) : null}
                </button>
              );
